                                           ...
                                           bool passthrough)
{
  ...
  int minBufferSize = CJNIAudioTrack::getMinBufferSize(safeSampleRate, channelMask, safeEncoding);
  if (minBufferSize <= 0)
    minBufferSize = static_cast<int>(safeSampleRate * safeChannels * 2 / 4);
  ...
  {
    CLog::Log(LOGERROR,
              "KodiTrueHdAudioTrackOutput::ConfigureTrueHd failed sampleRate={} channels={} encoding={}",
              safeSampleRate,
              safeChannels,
              safeEncoding);
    return false;
...
int KodiTrueHdAudioTrackOutput::WriteBlocking(const uint8_t* data, int size)
{
  if (!track_ || data == nullptr || size <= 0)
    return 0;
