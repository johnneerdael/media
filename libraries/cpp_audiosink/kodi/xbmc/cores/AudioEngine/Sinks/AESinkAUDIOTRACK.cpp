...
static std::string FormatProbeLog(const char* mode,
                                  int sampleRate,
                                  int channelMask,
                                  int encoding,
                                  int minBufferSize,
                                  bool supported)
{
  ...
  stream << "VerifySinkConfiguration mode=" << mode << " sampleRate=" << sampleRate
         << " channelMask=0x" << std::hex << channelMask << std::dec
         << " encoding=" << encoding << " minBufferSize=" << minBufferSize
                                                ...
                                                bool isRaw)
{
  ...
  CLog::Log(LOGDEBUG,
            FormatProbeLog(isRaw ? "RAW" : "IEC/PCM", sampleRate, channelMask, encoding,
                           minBufferSize, supported));
...
void CAESinkAUDIOTRACK::UpdateAvailablePassthroughCapabilities(bool isRaw)
{
  ...
  else
  {
    ...
    if (CJNIAudioFormat::ENCODING_IEC61937 != -1)
    {
      ...
      CLog::Log(LOGINFO,
                std::string("IEC61937 baseline probe stereo@48000 result=") +
                    (supports_iec ? "supported" : "unsupported"));
      if (supports_iec)
      {
        ...
        if (supports_192khz)
        {
          ...
          const bool supports_iec_stereo_192 =
              VerifySinkConfiguration(192000, CJNIAudioFormat::CHANNEL_OUT_STEREO,
                                      CJNIAudioFormat::ENCODING_IEC61937);
          CLog::Log(LOGINFO,
                    std::string("IEC61937 stereo@192000 result=") +
                        (supports_iec_stereo_192 ? "supported" : "unsupported"));
          if (supports_iec_stereo_192)
          {
            m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_EAC3);
            m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_DTSHD);
          ...
          const bool supports_iec_surround_192 =
              VerifySinkConfiguration(192000, atChannelMask, CJNIAudioFormat::ENCODING_IEC61937);
          std::ostringstream surround_probe;
          surround_probe << "IEC61937 surround@192000 channelMask=0x" << std::hex << atChannelMask
                         << std::dec << " result="
                         << (supports_iec_surround_192 ? "supported" : "unsupported");
          CLog::Log(LOGINFO, surround_probe.str());
          if (supports_iec_surround_192)
          {
            m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_DTSHD_MA);
            m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_TRUEHD);
        ...
        {
          CLog::Log(LOGINFO, "IEC61937 high-rate probes skipped because 192 kHz PCM support is unavailable");
        }
      }
      else
      {
        CLog::Log(LOGWARNING, "IEC61937 passthrough probe failed; sink will enumerate RAW-only passthrough");
      }
    }
  }
}
