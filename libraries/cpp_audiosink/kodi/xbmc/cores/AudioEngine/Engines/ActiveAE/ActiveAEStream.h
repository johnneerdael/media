...
class CActiveAEMediaStreamAdapter
{
public:
  void Configure(const AEAudioFormat& requestedFormat);
  void Reset();
  bool IsPassthrough() const { return m_requestedFormat.m_dataFormat == AE_FMT_RAW; }
  bool HasResolvedPassthroughFormat() const;
  AEAudioFormat GetResolvedFormat() const;
  const AEAudioFormat& GetRequestedFormat() const { return m_requestedFormat; }
  void FinalizeParser();

  // Parser-only access-unit splitting for passthrough.
  // Feeds data to the parser, returns bytes consumed from input.
  // If a complete access unit is parsed, outData/outSize are set.
  // outPtsUs and outDurationUs are set for the parsed unit.
  // Call in a loop until all input is consumed.
  int FeedPassthrough(const uint8_t* data, int size, int64_t presentationTimeUs,
                      const uint8_t** outData, unsigned int* outSize,
                      int64_t* outPtsUs, int64_t* outDurationUs);

  // Process any leftover parser backlog. Returns true if a unit was produced.
  bool DrainBacklog(const uint8_t** outData, unsigned int* outSize,
                    int64_t* outPtsUs, int64_t* outDurationUs);

  bool HasBacklog() const { return !m_parserBacklog.empty(); }

  int64_t DurationUsForWrite(int bytesWritten) const;

private:
  static constexpr int64_t NO_PTS = std::numeric_limits<int64_t>::min();

  int64_t ParserDurationUs();
  void EmitParsedUnit(const uint8_t* data, unsigned int size,
                      int64_t* outPtsUs, int64_t* outDurationUs);

  AEAudioFormat m_requestedFormat;
  CAEStreamParser m_streamParser;
  std::vector<uint8_t> m_parserBacklog;
  int64_t m_nextPacketPtsUs = 0;
  bool m_nextPacketPtsValid = false;
};
}
