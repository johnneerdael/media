...
namespace
{
bool HasValidDuration(const CAEStreamInfo& streamInfo)
{
  return streamInfo.m_type != CAEStreamInfo::STREAM_TYPE_NULL && streamInfo.m_sampleRate > 0 &&
         streamInfo.m_frameSize > 0;
}

int64_t StreamDurationUs(const CAEStreamInfo& streamInfo)
{
  if (!HasValidDuration(streamInfo))
    return 0;
  return static_cast<int64_t>(streamInfo.GetDuration() * 1000.0);
}
} // namespace
  ...
  while(copied < frames)
  {
    ...
    if (m_currentBuffer)
    {
      ...
      bool rawPktComplete = false;
      {
        ...
        {
          m_bufferedTime += static_cast<float>(StreamDurationUs(m_format.m_streamInfo)) / 1000000.0f;
          m_currentBuffer->pkt->nb_samples += minFrames;
          rawPktComplete = true;
        }
      }
...
void CActiveAEMediaStreamAdapter::Configure(const AEAudioFormat& requestedFormat)
{
  m_requestedFormat = requestedFormat;
  Reset();
}

void CActiveAEMediaStreamAdapter::Reset()
{
  m_streamParser.Reset();
  m_parserBacklog.clear();
  m_nextPacketPtsUs = 0;
  m_nextPacketPtsValid = false;
}

bool CActiveAEMediaStreamAdapter::HasResolvedPassthroughFormat() const
{
  if (!IsPassthrough())
    return true;

  const CAEStreamInfo& streamInfo = m_streamParser.GetStreamInfo();
  return streamInfo.m_type != CAEStreamInfo::STREAM_TYPE_NULL && streamInfo.m_sampleRate > 0;
}

AEAudioFormat CActiveAEMediaStreamAdapter::GetResolvedFormat() const
{
  AEAudioFormat format = m_requestedFormat;
  if (!IsPassthrough())
    return format;

  const CAEStreamInfo& streamInfo = m_streamParser.GetStreamInfo();
  if (streamInfo.m_type != CAEStreamInfo::STREAM_TYPE_NULL && streamInfo.m_sampleRate > 0)
    format.m_streamInfo = streamInfo;

  return format;
}

void CActiveAEMediaStreamAdapter::FinalizeParser()
{
  // Drain any remaining backlog through the parser (no-op if empty).
  while (!m_parserBacklog.empty())
  {
    uint8_t* parsed = nullptr;
    unsigned int parsedSize = 0;
    const unsigned int consumed =
        m_streamParser.AddData(m_parserBacklog.data(), m_parserBacklog.size(), &parsed, &parsedSize);
    if (consumed > 0)
      m_parserBacklog.erase(m_parserBacklog.begin(), m_parserBacklog.begin() + consumed);
    if (consumed == 0 && parsedSize == 0)
      break;
  }
}

int CActiveAEMediaStreamAdapter::FeedPassthrough(
    const uint8_t* data, int size, int64_t presentationTimeUs,
    const uint8_t** outData, unsigned int* outSize,
    int64_t* outPtsUs, int64_t* outDurationUs)
{
  *outData = nullptr;
  *outSize = 0;
  ...
  if (presentationTimeUs != NO_PTS && !m_nextPacketPtsValid)
  {
    m_nextPacketPtsUs = presentationTimeUs;
    m_nextPacketPtsValid = true;
  }

  // If there is backlog, append new data and parse from backlog.
  if (!m_parserBacklog.empty())
  {
    m_parserBacklog.insert(m_parserBacklog.end(), data, data + size);
    int consumed = size; // all input bytes are now in backlog
    ...
    const unsigned int used =
        m_streamParser.AddData(m_parserBacklog.data(), m_parserBacklog.size(), &parsed, &parsedSize);
    if (used > 0)
      m_parserBacklog.erase(m_parserBacklog.begin(), m_parserBacklog.begin() + used);

    if (parsedSize > 0)
    {
      *outData = parsed;
      *outSize = parsedSize;
      EmitParsedUnit(parsed, parsedSize, outPtsUs, outDurationUs);
    }
    return consumed;
  }

  // No backlog — parse directly from input.
  uint8_t* parsed = nullptr;
  unsigned int parsedSize = 0;
  const unsigned int used =
      m_streamParser.AddData(const_cast<uint8_t*>(data), size, &parsed, &parsedSize);
  ...
  // Parser consumed nothing — put remaining input in backlog for next call.
  if (size > 0)
    m_parserBacklog.assign(data, data + size);
  return size; // report all bytes consumed (held in backlog)
}

bool CActiveAEMediaStreamAdapter::DrainBacklog(
    const uint8_t** outData, unsigned int* outSize,
    int64_t* outPtsUs, int64_t* outDurationUs)
{
  *outData = nullptr;
...
void CActiveAEMediaStreamAdapter::EmitParsedUnit(
    const uint8_t* /* data */, unsigned int /* size */,
    int64_t* outPtsUs, int64_t* outDurationUs)
{
  const int64_t durationUs = ParserDurationUs();
  const int64_t ptsUs = m_nextPacketPtsValid ? m_nextPacketPtsUs : NO_PTS;
  ...
  *outDurationUs = durationUs;
  ...
  if (ptsUs != NO_PTS)
  {
    m_nextPacketPtsUs = ptsUs + durationUs;
    m_nextPacketPtsValid = true;
  }
}

int64_t CActiveAEMediaStreamAdapter::ParserDurationUs()
{
  const int64_t parserDurationUs = StreamDurationUs(m_streamParser.GetStreamInfo());
  if (parserDurationUs > 0)
    return parserDurationUs;
  return DurationUsForWrite(0);
}

int64_t CActiveAEMediaStreamAdapter::DurationUsForWrite(int bytesWritten) const
{
  if (m_requestedFormat.m_dataFormat == AE_FMT_RAW)
    return StreamDurationUs(m_requestedFormat.m_streamInfo);

  if (m_requestedFormat.m_frameSize == 0 || m_requestedFormat.m_sampleRate == 0)
    return 0;

  const int frames = bytesWritten / static_cast<int>(m_requestedFormat.m_frameSize);
  return (static_cast<int64_t>(frames) * 1000000LL) / m_requestedFormat.m_sampleRate;
}
