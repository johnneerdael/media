...
double SafeStreamDurationSeconds(const CAEStreamInfo& streamInfo)
{
  if (streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_NULL || streamInfo.m_sampleRate == 0)
    return 0.0;
  return streamInfo.GetDuration() / 1000.0;
}

double SafeStreamDurationMs(const CAEStreamInfo& streamInfo)
{
  if (streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_NULL || streamInfo.m_sampleRate == 0)
    return 0.0;
...
void CEngineStats::GetDelay(AEDelayStatus& status)
{
  ...
  if (m_pcmOutput)
    ...
    status.delay += static_cast<double>(m_bufferedSamples) *
                    SafeStreamDurationSeconds(m_sinkFormat.m_streamInfo);
}
    ...
    if (str.m_streamId == stream->m_id)
    {
      ...
      for(itBuf=stream->m_processingSamples.begin(); itBuf!=stream->m_processingSamples.end(); ++itBuf)
      {
        if (m_pcmOutput)
          ...
          delay += static_cast<float>(SafeStreamDurationSeconds(m_sinkFormat.m_streamInfo));
      }
...
float CEngineStats::GetWaterLevel()
{
  ...
  if (m_pcmOutput)
    ...
    return static_cast<float>(m_bufferedSamples) *
           static_cast<float>(SafeStreamDurationSeconds(m_sinkFormat.m_streamInfo));
}
  ...
  for (it = m_streams.begin(); it != m_streams.end(); ++it)
  {
    ...
    if (!(*it)->m_drain)
    {
      ...
      if ((*it)->m_inputBuffers->m_format.m_dataFormat == AE_FMT_RAW)
        buftime = static_cast<float>(SafeStreamDurationSeconds((*it)->m_inputBuffers->m_format.m_streamInfo));
...
CSampleBuffer* CActiveAE::SyncStream(CActiveAEStream *stream)
{
  ...
  if (stream->m_syncState == CAESyncInfo::AESyncState::SYNC_MUTE)
  {
    ...
    if (m_mode == MODE_RAW)
    {
      ...
      buf->pkt->pause_burst_ms =
          SafeStreamDurationMs(stream->m_processingBuffers->m_inputFormat.m_streamInfo);
    }
    else
    {
  ...
  else if (stream->m_syncState == CAESyncInfo::AESyncState::SYNC_ADJUST)
  {
    if (error > 0)
    {
      ...
      if (ret)
      {
        ...
        if (m_mode == MODE_RAW)
        {
          ...
          const double streamDurationMs = SafeStreamDurationMs(stream->m_format.m_streamInfo);
          if (streamDurationMs > 0.0 && error > streamDurationMs)
            ret->pkt->pause_burst_ms = streamDurationMs;
    ...
    else
    {
      ...
      {
        const double streamDurationMs = SafeStreamDurationMs(stream->m_format.m_streamInfo);
        if (streamDurationMs > 0.0 && -error > streamDurationMs / 2)
        {
          stream->m_syncError.Correction(streamDurationMs);
          error += streamDurationMs;
          buf->pkt->nb_samples = 0;
        }
      }
      else
