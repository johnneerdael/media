 ...
 *  - Added std::vector convenience overloads for JNI module callers.
 *  - Preserved Kodi constants and packing logic as the authoritative source.
 */
...
public:
  ...
  static int PackAC3ToVector(uint8_t* data, unsigned int size, std::vector<uint8_t>* dest);
  static int PackEAC3ToVector(uint8_t* data, unsigned int size, std::vector<uint8_t>* dest);
  static int PackTrueHDToVector(const uint8_t* data, unsigned int size, std::vector<uint8_t>* dest);
...
private:
  ...
  enum IEC61937DataType
  {
    ...
    IEC61937_TYPE_DTS1 = 0x0B,
    IEC61937_TYPE_DTS2 = 0x0C,
    IEC61937_TYPE_DTS3 = 0x0D,
